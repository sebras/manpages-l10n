...
timestamp=$(TZ='Europe/Amsterdam' date "+%d %m %Y, %H:%M")
...
manpage_sections=$(find ../po/nl/man* -maxdepth 1 -type d | cut -d/ -f4 | LC_ALL=C sort)
...
<html lang="en">
  <head>
    ...
    <title>Nederlandse vertaling van man pages</title>
  </head>
  <body>
    <div class="container-fluid">
      <h1>Lijst van bestanden, die niet volledig vertaald zijn</h1>
      <p>Tijd: $timestamp</p>
      <p>
        <a class="btn btn-primary" href="untranslated.html">Niet vertaalde bestanden</a>
      </p>
END_OF_HEADER
...
for distribution in $distributions; do
  ...
  <html lang="en">
    ...
    <body>
        ...
        <p>Tijd: $timestamp</p>
        <p>
          <a class="btn btn-primary" href="index.html">Overzicht</a>
        </p>
        <p>
          <a class="btn btn-primary" href="https://salsa.debian.org/manpages-l10n-team/manpages-l10n">Git repository bekijken</a>
        </p>
END_OF_HEADER
  ...
  for manpage_section in $manpage_sections; do
  	section_count=0
    table_rows=""
    translations=$(find "../po/nl/$manpage_section" -name "*.po" | LC_ALL=C sort)
    ...
    if [ $section_count -gt 0 ]; then
			...
			<table class="table table-striped table-bordered table-sm">
			  <thead class="thead-dark">
			    <tr>
			      <th scope="col" width="25%">Naam</th>
			      <th scope="col" width="10%">Percentage</th>
			      <th scope="col" width="15%">Vertalingen tot 80%</th>
			      <th scope="col" width="50%">Statistiek</th>
			    </tr>
			  </thead>
			  <tbody>
EOF_TABLE
      ...
      echo '<div class="alert alert-primary" role="alert">' >> $distribution.html
			if [ $section_count -eq 1 ]; then
				echo "1 bestand is niet volledig vertaald." >> $distribution.html
			else
				echo "$section_count bestanden zijn niet volledig vertaald." >> $distribution.html
			fi
...
EOF_TABLE
  ...
  echo "Totaal zijn " >> untranslated.html
  (wc -l  ../upstream/$distribution/untranslated.txt | cut -d" " -f1) >> untranslated.html
  echo " bestanden niet volledig vertaald." >> untranslated.html
