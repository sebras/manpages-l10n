...
grep -a -v "Macedonian translation of manpages" |
...
case $(basename "$translation") in
	tar.1.po ) echo ".Sh ПРЕВОД" >> "$addendum" ;;
	* ) echo ".SH ПРЕВОД" >> "$addendum" ;;
esac
echo "Македонскиот превод на оваа рачна страница е создаден од" >> "$addendum"
