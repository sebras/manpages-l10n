...
if [ ! -f $mandir/$manpage.po ]; then
	...
	msginit --no-translator --locale="sr" -i "$potfile" -o "$pofile"
	...
	echo "# Serbian translation of manpages" > "$pofile"
	...
	sed -i -e "s/^\"Project-Id-Version: manpages-sr .*$/\"Project-Id-Version: manpages-l10n\\\n\"/" "$pofile"
fi
