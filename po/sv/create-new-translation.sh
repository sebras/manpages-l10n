...
if [ ! -f $mandir/$manpage.po ]; then
	...
	msginit --no-translator --locale="sv" -i "$potfile" -o "$pofile"
	...
	echo "# Swedish translation of manpages" > "$pofile"
	...
	sed -i -e "s/^\"Project-Id-Version: manpages-sv .*$/\"Project-Id-Version: manpages-l10n\\\n\"/" "$pofile"
fi
