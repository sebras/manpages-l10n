...
if [ ! -f $mandir/$manpage.po ]; then
	...
	msginit --no-translator --locale="el" -i "$potfile" -o "$pofile"
	...
	echo "# Greek translation of manpages" > "$pofile"
	...
	sed -i -e "s/^\"Project-Id-Version: manpages-el .*$/\"Project-Id-Version: manpages-l10n\\\n\"/" "$pofile"
fi
