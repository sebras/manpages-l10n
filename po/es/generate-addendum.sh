...
grep -a -v "Spanish translation of manpages" |
...
case $(basename "$translation") in
	tar.1.po ) echo ".Sh TRADUCCIÓN" >> "$addendum" ;;
	* ) echo ".SH TRADUCCIÓN" >> "$addendum" ;;
esac
echo "La traducción al español de esta página del manual fue creada por" >> "$addendum"
