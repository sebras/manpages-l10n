...
if [ ! -f $mandir/$manpage.po ]; then
	...
	msginit --no-translator --locale="fi" -i "$potfile" -o "$pofile"
	...
	echo "# Finnish translation of manpages" > "$pofile"
	...
	sed -i -e "s/^\"Project-Id-Version: manpages-fi .*$/\"Project-Id-Version: manpages-l10n\\\n\"/" "$pofile"
fi
